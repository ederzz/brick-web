...
const getCodes = (bui) => {
  ...
  if(bui.ui && bui.brick) {
    ...
    if(/\/</.test(bui.ui)) {
      ...
      htmlCode = codeList[bui.ui] && codeList[bui.ui].html && codeList[bui.ui].category === 'template' ? codeList[bui.ui].html : bui.ui
    }
    ...
    for(const modelName in bui.brick) {
      let reactTag = getCodes(bui.brick[modelName])
      reactTag = reactTag.replace(/\w+/g, function(word){
        return word.substring(0,1).toUpperCase()+word.substring(1);}
      );
      reactTag = reactTag.replace(/-/g,'')
      //htmlCode = htmlCode.replace(`||${modelName}||`,thisHtmlCode)
      // 替换掉js代码里 ||model|| 为 <Model />
      // 1 有参数 <ModelName ||...params|| />
      // 2 无参数 使用 html 里面的 jsx
      // 3 缺省 <ModelName />
      console.log('modelName 1',modelName, reactTag, jsCode)
      jsCode = jsCode.replace(`<${modelName}`,`<${reactTag}`)
      jsCode = jsCode.replace(`</${modelName}>`,`</${reactTag}>`)
    }
    ...
    if(/\/s$/.test(modelName)) {
      ...
      htmlCode = modelName //codeList[modelName] && codeList[modelName].html ? codeList[modelName].html : `没有找到 ||${modelName}|| 模块`
      jsCode = codeList[modelName] && codeList[modelName].js ? codeList[modelName].js : ''
      cssCode = codeList[modelName] && codeList[modelName].css ? codeList[modelName].css : ''
    }
