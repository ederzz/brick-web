...
export default class WorksEditor extends React.Component {
  ...
  getCode = () => {
    ...
    httpAgent.get(`/works/info?id=${id}`).then(res=>{
  ...
  render() {
    ...
    console.log('web code',this.state)
    ...
    return(<div className="wrap clearfix">
      <WorkEditor type="view" httpAgent={httpAgent} history={history} initCode={initCode} id={id} editorInfo={editorInfo}  />
    </div>)
